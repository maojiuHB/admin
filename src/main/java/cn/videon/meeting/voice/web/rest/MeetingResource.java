...
public class MeetingResource {
     ...
     * or with status 500 (Internal Server Error) if the meetingDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/meetings")
    @Timed
    ...
    @GetMapping("/meetings")
    @Timed
    public ResponseEntity<List<MeetingDTO>> getAllMeetings(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, String.format("/api/meetings?eagerload=%b", eagerload));
